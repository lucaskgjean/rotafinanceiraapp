...
import { AppConfig, DEFAULT_CONFIG, DailyEntry } from '../types';
...
interface SettingsProps {
  ...
  onImport: (entries: DailyEntry[], config?: AppConfig) => void;
}

const Settings: React.FC<SettingsProps> = ({ config, entries, onChange, onImport }) => {
      ...
      try {
        ...
        if (data.entries && Array.isArray(data.entries)) {
          ...
          if (window.confirm(`Detectado backup completo com ${data.entries.length} registros. Restaurar histórico e configurações?`)) {
            onImport(data.entries, data.config);
          }
